                        ...
                        <div class="mb-4 row">
                            ...
                            <div class="col-sm-12">
                                <select name="target_page" id="target_page" class="form-select @error('target_page') is-invalid @enderror" required>
                                    <option value="all" {{ old('target_page', $announcement->target_page ?? 'all') == 'all' ? 'selected' : '' }}>All Pages</option>
                                    <option value="homepage" {{ old('target_page', $announcement->target_page ?? 'all') == 'homepage' ? 'selected' : '' }}>Homepage Only (Landing 2)</option>
                                    <option value="career" {{ old('target_page', $announcement->target_page ?? 'all') == 'career' ? 'selected' : '' }}>Career Pages</option>
                                    <option value="promo" {{ old('target_page', $announcement->target_page ?? 'all') == 'promo' ? 'selected' : '' }}>Promotion Pages</option>
                                    <option value="event" {{ old('target_page', $announcement->target_page ?? 'all') == 'event' ? 'selected' : '' }}>Event Pages</option>
                                </select>
                                @error('target_page')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
