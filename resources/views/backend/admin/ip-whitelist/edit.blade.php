...
@section('page-title', 'Edit IP Whitelist')
@section('page-link', route('admin.ip-whitelist.edit', $ip->id))

@section('content')
<div class="row mb-4 align-items-center">
    <div class="col">
        <p class="text-muted mb-0">Ubah detail IP Address yang diizinkan untuk mengakses subdomain.</p>
    </div>
    <div class="col-auto">
        ...
        <div class="card">
            <div class="card-header py-3">
                <h6 class="mb-0 fw-semibold">
                    <i class="ti ti-shield-lock me-2 text-primary"></i>Edit IP Whitelist
                </h6>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.ip-whitelist.update', $ip->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <select name="subdomain" id="subdomain" class="form-select @error('subdomain') is-invalid @enderror" required>
                            @foreach($subdomains as $sd)
                                <option value="{{ $sd }}" {{ old('subdomain', $ip->subdomain) == $sd ? 'selected' : '' }}>
                    ...
                    <div class="d-flex gap-2 justify-content-end">
                        ...
                        <button type="submit" class="btn btn-primary hstack gap-2">
                            <i class="ti ti-check fs-5"></i> Perbarui IP Whitelist
                        </button>
                    </div>
                </form>
            </div>
