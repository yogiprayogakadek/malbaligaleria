    ...
    <style>
        ...
        *::-webkit-scrollbar {
        ...
        .tenant-card:hover {
            ...
            box-shadow: 0 12px 32px rgba(95, 207, 218, 0.15);
        }
        ...
        /* Map Pin Styles - UPDATED FOR PIXEL POSITIONING */
        .map-pin {
            position: absolute;
            width: 32px;
            height: 32px;
            ...
            transition: all 0.3s ease;
            box-shadow: 0 4px 12px rgba(95, 207, 218, 0.4);
            z-index: 10;
        ...
        @keyframes pulse {
            ...
            0%,
            100% {
                box-shadow: 0 4px 12px rgba(95, 207, 218, 0.4), 0 0 0 0 rgba(95, 207, 218, 0.7);
            }

            50% {
                box-shadow: 0 4px 12px rgba(95, 207, 218, 0.4), 0 0 0 10px rgba(95, 207, 218, 0);
            }
        }
        ...
        .map-pin:hover {
            ...
            background: #4db8c3;
            box-shadow: 0 6px 20px rgba(95, 207, 218, 0.6);
            z-index: 20;
            animation: none;
        }
...
<body>
                ...
                <aside class="filter-sidebar">
                    ...
                    <div class="filter-body">
                        ...
                        <div class="filter-stats">
                            <div class="filter-stats-number" id="tenantCount">16</div>
    ...
    <script>
        ...
        // PENTING: mapCoords menggunakan koordinat PIXEL dari gambar ASLI
        // Gunakan enableCoordinatePicker() untuk mendapatkan koordinat yang tepat
        ...
        // ===== HELPER FUNCTION - Calculate Responsive Position =====
        ...
        document.getElementById('sidebarSearch').addEventListener('input', filterTenants);
        ...
        // ===== UPDATE MAP VIEW FUNCTION - WITH RESPONSIVE PINS =====
        function updateMapView() {
            const mapContainer = document.getElementById('mapContainer');
            const selectedFloor = document.getElementById('floorFilter').value;

            const floorTenants = tenants.filter(t =>
                !selectedFloor || t.floor === selectedFloor
            );

            const floorKey = selectedFloor === '1st Floor' ? 'floor1' :
                selectedFloor === '2nd Floor' ? 'floor2' : currentFloorMap;

            mapContainer.innerHTML = `
                <div class="map-wrapper" id="mapWrapper" style="position: relative;">
                    <img src="${floorMaps[floorKey]}"
                         alt="Mall Floor Plan"
                         id="floorMapImage"
                         style="width: 100%; height: auto; display: block;">
                </div>
                <div class="map-stats">
                    <div class="map-stats-number">${floorTenants.length}</div>
            ...
            const addPins = () => {
                ...
                console.log('Map Size:', currentWidth, 'x', currentHeight);
                console.log('Adding', floorTenants.length, 'pins');

                // Add pins with calculated positions
                    ...
                    if (tenant.mapCoords && tenant.mapOriginalSize) {
                        ...
                        pin.addEventListener('click', function(e) {
                            ...
                            if (window.innerWidth <= 768) {
                                toggleMobileTooltip(tenant, this);
                            } else {
        ...
        function showMapTooltip(tenant, pinElement) {
            ...
            const arrowHTML = tooltip.querySelector('.tooltip-arrow') ? '' : '<div class="tooltip-arrow"></div>';
            tooltip.innerHTML = `
                ${arrowHTML}
                <h4>${tenant.name}</h4>
                <p>📍 ${tenant.category}</p>
                <p>🕒 ${tenant.hours}</p>
                <span class="unit-badge">Unit ${tenant.unit}</span>
            `;
        ...
        console.log('- Auto-resize on window change');
        console.log('- Mobile responsive');
        console.log('- Coordinate picker tool included');
        console.log('%cRun enableCoordinatePicker() to get coordinates', 'color: #666; font-size: 12px;');
    </script>
</body>
