...
<head>
    ...
    <title>Asset by Category - {{ date('Y-m-d') }}</title>
    ...
    <style>
        ...
        .sum-item .sum-val { font-size: 20px; font-weight: 700; color: #1e40af; }
...
<body>
    ...
    <div class="header">
        <div class="logo-area">
            ...
            <p>Asset &amp; Devices Inventory — Kelompok Per Kategori</p>
        </div>
        <div class="meta-area">
            <p>Date Generated: <strong>{{ date('d F Y H:i') }}</strong></p>
            <p>Format: <strong>Grouped by Category</strong></p>
        </div>
    </div>
    ...
    <div class="filter-badge-row">
        ...
        <div class="filter-badge">
            Total Kategori: <strong>{{ $grouped->count() }}</strong>
        </div>
    </div>
    ...
    <div class="summary-bar">
        ...
        <div class="sum-item">
            ...
            <span class="sum-lbl">Kategori</span>
        </div>
        <div class="sum-divider"></div>
        <div class="sum-item">
    ...
    @foreach($grouped as $categoryName => $catItems)
        <div class="cat-section {{ $categoryName === 'Tanpa Kategori' ? 'cat-uncategorized' : '' }}">
            <div class="cat-header">
                <h3>📁 {{ $categoryName }}</h3>
                <span class="cat-count">{{ $catItems->count() }} aset</span>
            </div>
            <table>
                <thead>
                    <tr>
                            ...
                            <td>
                                ...
                                @if(!empty($item->specs) && is_array($item->specs))
                                    <div style="font-size:9px;color:#64748b;margin-top:2px">
                                        @foreach(array_slice($item->specs, 0, 3) as $k => $v)
